...
export default function Navbar({ 
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (!target.closest('.search-container-boundary')) {
  ...
  const renderSearchBox = (isMobile: boolean) => (
    <>
      ...
      <div className={`search-container-boundary relative items-center flex-1 ${isMobile ? "flex lg:hidden" : "hidden lg:flex order-1 lg:order-3 mx-2"}`}>
              <div className={`flex items-center bg-[#1d4ed8] border border-blue-400/50 ${isSearchExpanded ? (isMobile ? "absolute right-0 w-40 sm:w-52 pr-1 z-50 shadow-xl" : "w-36 md:w-40 sm:w-56 pr-2") : "w-6 sm:w-7 md:w-8 hover:bg-[#1e40af]"} transition-all duration-300 rounded h-6 sm:h-7 md:h-8 overflow-hidden`}>
                <button
                  type="button"
                ...
                {isSearchExpanded && (
                  <button
                    ...
                    className="text-slate-900 dark:text-slate-100 hover:text-sky-600 px-1.5 text-xs font-bold cursor-pointer transition-colors focus:outline-none flex items-center justify-center"
              ...
              {isSearchExpanded && searchQuery.trim().length >= 2 && (
                <>
                  ...
                  <div className="absolute top-full right-0 mt-2 w-72 sm:w-80 bg-white dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800/60 rounded-xl shadow-2xl z-50 py-3 max-h-96 overflow-y-auto text-left animate-in fade-in-50 zoom-in-95 duration-150">
                    <p className="text-[10px] font-mono tracking-widest text-slate-900 dark:text-slate-100 uppercase px-3 pb-1.5 border-b border-slate-200/80 dark:border-slate-800/60/80 mb-2 font-bold">
                      ...
                      <div className="mb-2.5">
                        ...
                        {matchedCategories.map((cat) => (
                          <button
                            ...
                            className="w-full text-left px-3 py-1.5 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-[#1e293b] text-xs font-semibold text-slate-900 dark:text-slate-900 hover:text-sky-600 transition flex items-center gap-2 cursor-pointer"
                          >
                      ...
                      <div className="mb-2.5">
                        ...
                        {filteredPackages.map((pkg: any) => (
                          <button
                            ...
                            className="w-full text-left px-3 py-2 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-[#1e293b] text-xs font-semibold text-slate-900 dark:text-slate-900 hover:text-sky-600 transition flex flex-col gap-0.5 cursor-pointer border-b border-slate-200 dark:border-slate-800/60 last:border-b-0"
                          >
                            <div className="flex items-center gap-1.5">
                              <Compass className="h-3 w-3 text-sky-450 shrink-0" />
                              <span className="truncate">{pkg.title}</span>
                            </div>
                            <span className="text-[10px] text-slate-900 dark:text-slate-100 font-normal truncate pl-4.5">{pkg.duration} • {pkg.price}</span>
                          </button>
                        ))}
                      </div>
                    )}
                      ...
                      <div className="mb-1">
                        ...
                        {filteredBlogs.map((blog: any) => (
                          <button
                            ...
                            className="w-full text-left px-3 py-2 hover:bg-white dark:bg-[#0b1120] text-xs font-semibold text-slate-900 dark:text-slate-900 hover:text-sky-600 transition flex flex-col gap-0.5 cursor-pointer border-b border-slate-200 dark:border-slate-800/60 last:border-b-0"
                          >
                            <div className="flex items-center gap-1.5">
                              <Sparkles className="h-3.5 w-3.5 text-blue-550 shrink-0" />
                              <span className="truncate">{blog.title}</span>
                            </div>
                            <span className="text-[10px] text-slate-900 dark:text-slate-100 font-normal truncate pl-4.5">{blog.readTime} • by {blog.author}</span>
                          </button>
                        ))}
                      </div>
                    )}
                    ...
                    {matchedCategories.length === 0 && filteredPackages.length === 0 && filteredBlogs.length === 0 && (
                      <div className="p-4 text-center text-xs text-slate-900 dark:text-slate-100">
                        No matchmaking results found.<br />
                        <span className="text-[10px] text-slate-900 dark:text-slate-100 mt-1.5 block">Try searching &quot;Parvat&quot;, &quot;Kailash&quot; or &quot;Heli&quot;</span>
                      </div>
                    )}
                  </div>
                </>
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 dark:bg-[#0b1120]/95 backdrop-blur-md shadow-lg border-b border-sky-500/10">
        ...
        <div className="w-full max-w-full mx-auto flex flex-wrap items-center justify-between md:justify-start gap-y-2 md:gap-0 px-1 md:px-8">
          ...
          <div className="relative order-3 md:order-5 shrink-0">
              <button
                ...
                className="flex items-center gap-1.5 cursor-pointer hover:text-sky-600 transition text-[11.5px] uppercase tracking-wider text-slate-900 dark:text-slate-100 bg-slate-200 dark:bg-[#1e293b] px-3 py-1 rounded border border-slate-300 dark:border-slate-700/60 font-medium whitespace-nowrap outline-none"
              >
                <Globe className="h-3 w-3 text-sky-500" />
                <span>{languages.find((l) => l.code === currentLanguage)?.label || "English"}</span>
                <span className="text-[7px] text-slate-900 dark:text-slate-100 ml-0.5">▼</span>
              </button>
              
              {isLangMenuOpen && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setIsLangMenuOpen(false)} />
                  <div className="absolute right-0 mt-1.5 w-44 bg-white dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800/60 rounded-lg shadow-2xl z-50 py-1.5 animate-in fade-in-50 zoom-in-95 duration-100 text-left">
                    <p className="text-[9px] font-mono tracking-widest text-slate-900 dark:text-slate-100 uppercase px-3 py-1 border-b border-slate-200/80 dark:border-slate-800/60/80 mb-1">
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 text-xs font-serif tracking-wide transition flex items-center justify-between ${
                          currentLanguage === l.code
                            ...
                            : "text-slate-900 dark:text-slate-100 hover:bg-white dark:bg-[#0b1120] hover:text-sky-600"
                        }`}
                      >
                        <span>{l.label}</span>
                        <span className="text-[10px] text-slate-900 dark:text-slate-100 font-sans font-medium">{l.nativeLabel}</span>
                      </button>
                    ))}
                  </div>
                </>
            ...
            <div className="hidden md:flex items-center justify-start md:justify-center gap-1.5 w-full md:w-auto border-t border-white/20 md:border-t-0 md:border-l md:border-slate-300/60 pt-1.5 md:pt-0 mt-1.5 md:mt-0 md:pl-3 pr-[100px] md:pr-0 -ml-1 md:ml-0.5 order-5 md:order-6 pb-0.5 md:pb-0 relative">
              {socialLinks.facebook && (
                <a href={socialLinks.facebook} target="_blank" rel="noopener noreferrer" className="p-1 rounded-full text-slate-900 dark:text-slate-100 hover:text-sky-500 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-[#1e293b] transition" title="Facebook">
              ...
              {socialLinks.instagram && (
                <a href={socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="p-1 rounded-full text-slate-900 dark:text-slate-100 hover:text-sky-500 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-[#1e293b] transition" title="Instagram">
              ...
              {socialLinks.linkedin && (
                <a href={socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="p-1 rounded-full text-slate-900 dark:text-slate-100 hover:text-sky-500 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-[#1e293b] transition" title="LinkedIn">
              ...
              {socialLinks.youtube && (
                <a href={socialLinks.youtube} target="_blank" rel="noopener noreferrer" className="p-1 rounded-full text-slate-900 dark:text-slate-100 hover:text-sky-500 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-[#1e293b] transition" title="YouTube">
              ...
              {socialLinks.pinterest && (
                <a href={socialLinks.pinterest} target="_blank" rel="noopener noreferrer" className="p-1 rounded-full text-slate-900 dark:text-slate-100 hover:text-sky-500 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-[#1e293b] transition" title="Pinterest">
              ...
              {socialLinks.reddit && (
                <a href={socialLinks.reddit} target="_blank" rel="noopener noreferrer" className="p-1 rounded-full text-slate-900 dark:text-slate-100 hover:text-sky-500 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-[#1e293b] transition" title="Reddit">
              ...
              {socialLinks.twitter && (
                <a href={socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="p-1 rounded-full text-slate-900 dark:text-slate-100 hover:text-sky-500 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-[#1e293b] transition" title="Twitter / X">
              ...
              <button
                ...
                className="md:hidden absolute right-2 text-[10.5px] font-bold text-slate-900 dark:text-slate-100 uppercase tracking-widest flex items-center justify-center gap-1.5 hover:text-sky-500 transition px-4 sm:px-5 py-1.5 rounded-full border border-slate-300 dark:border-slate-700/60 dark:hover:border-sky-500 bg-white dark:bg-[#0f172a] shadow-sm whitespace-nowrap min-w-max"
              >
      ...
      {/* Main Core Navbar */}
      <div className="w-full max-w-full mx-auto px-4 md:px-8 py-2 md:py-2.5 flex justify-between items-center">
        {/* Dynamic Image Logo */}
        <div className="flex items-center gap-3">
          <div className="relative group cursor-pointer flex items-center gap-3" onClick={() => {
            ...
            <img 
              ...
              className="relative w-9 h-9 md:w-12 md:h-12 rounded-full border border-sky-500/30 object-contain shadow-lg"
              referrerPolicy="no-referrer"
            />
            <div className="hidden sm:flex flex-col">
              <span className="font-sans text-lg md:text-xl tracking-wider font-extrabold text-slate-900 dark:text-slate-100 leading-none drop-shadow-sm">
                ADI KAILASH <span className="text-sky-500">TIRATH</span>
              </span>
              <span className="text-[8.5px] md:text-[9.5px] tracking-[0.2em] md:tracking-[0.25em] text-slate-900 dark:text-slate-100 uppercase font-sans mt-1">
                 {tagline}
               </span>
            </div>
          </div>
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {activeCategory !== "home" && (
            <button
              ...
              className="mr-3 px-2 py-1.5 rounded-md text-xs font-bold tracking-wide transition-all bg-white dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800/60 hover:border-sky-300 text-sky-600 hover:text-slate-900 dark:text-slate-100 flex items-center gap-1.5 cursor-pointer shadow-md select-none"
            >
              <svg className="h-3 w-3 stroke-[2.5]" viewBox="0 0 24 24" fill="none" stroke="currentColor"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-2.5 py-1.5 rounded-md text-xs font-medium tracking-wide transition-all ${
                activeCategory === cat.key
                  ? "bg-gradient-to-r from-sky-600/20 to-sky-500/10 text-sky-600 border border-sky-500/35"
                  : "text-slate-900 dark:text-slate-100 hover:text-sky-600 hover:bg-zinc-90 w/10 border border-transparent"
              }`}
            >
              {cat.label}
            </button>
        ...
        <div className="hidden lg:flex items-center gap-3">
          <button
            ...
            className="bg-amber-600 hover:bg-amber-500 text-white text-[11px] font-bold font-sans tracking-wider px-3.5 py-2 rounded-md shadow-lg shadow-amber-600/15 border border-amber-400 hover:scale-[1.02] transition-all cursor-pointer inline-flex items-center gap-1.5"
          >
          ...
          <button
            onClick={toggleTheme}
            className="p-2 text-slate-900 dark:text-slate-100 hover:text-sky-600 bg-slate-100 dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800/60 rounded-md transition cursor-pointer"
            title="Toggle Theme"
          >
        ...
        <div className="lg:hidden flex items-center gap-2">
          ...
          <button
            onClick={toggleTheme}
            className="p-1 text-slate-900 dark:text-slate-100 hover:text-sky-600 bg-slate-100 dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800/60 rounded transition cursor-pointer"
            title="Toggle Theme"
          >
          ...
          <button
            onClick={onOpenConsultant}
            className="p-1 text-amber-600 bg-amber-500/10 border border-amber-500/30 rounded"
            title="Ask AI Guide"
          >
          ...
          <button
            ...
            className="p-1 text-slate-900 dark:text-slate-100 hover:text-sky-600 bg-white dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800/60 rounded transition"
          >
      ...
      {isOpen && (
        <div className="lg:hidden bg-white dark:bg-[#0b1120] border-t border-slate-300 dark:border-slate-700/60 px-4 py-3 flex flex-col gap-2 shadow-2xl transition duration-300 animate-in fade-in-50">
          <p className="text-[10px] font-mono tracking-widest text-slate-900 dark:text-slate-100 uppercase pb-1 border-b border-slate-300 dark:border-slate-700/60">
          ...
          <div className="pt-2 border-t border-slate-300 dark:border-slate-700/60 flex flex-col gap-2 mt-1">
            <button
              onClick={() => {
                setIsOpen(false);
                onOpenConsultant();
